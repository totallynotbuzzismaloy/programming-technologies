...
namespace tp_5Lab
{
    public partial class Form1 : Form
    {
        ...
        private void printTheInfo(double?[,] outputArray, double x0, double step)
        {
            ...
            {
                var myX = x0 + step * i;
                richTextBox1.Text += $"{Math.Round(myX, 3), 5} ";
                for (var j = 0; j < outputArray.GetUpperBound(1) + 1; ++j)
                {
                    if (outputArray[i, j] != null)
                        ...
                        richTextBox1.Text +=
                            $"  {"  Null", 7}  "; //$"The func doesnt exist in ({myX,5};{i + 1,5}){Environment.NewLine}";
                }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               // richTextBox1.Text += $"{myArray[0]}\t {myArray[1]}\t{myArray[2]}\t{myArray[3]}{Environment.NewLine}";
                ...
                var tempFileName = $"G{selectedIndex + 1:####}.dat";
                ...
                for (var i = 0; i < outputArray.GetUpperBound(0) + 1; ++i)
                {
                    ...
                    {
                        if (outputArray[i, j] == null)
                            _myCreateFiles.myCreateErrorFile(tempFileName,myX, j+1, "Arithmetic Exception");
                    }
                }
                var amountX = outputArray.GetUpperBound(0) + 1;
                var amountY = outputArray.GetUpperBound(1) + 1;
                _myCreateFiles.DatFileToCreate(tempFileName,amountX, amountY, outputArray, myArray[0], myArray[2]);
